...
日期：2020/05/25
地点：西城区桥湾
说明：训练常基于dark-net的YOLOv3网络，目标检测(自己编写的简化版)
"""
...
class YOLOv3Tiny(object):
    ...
    def conv_bn(self,
        ...
        conv = fluid.layers.conv2d(input=input,
                                   ...
                                   act=None,
                                   groups=num_groups,
                                   use_cudnn=use_cudnn,  # 是否使用cudnn对cuda加速
                                   param_attr=param_attr,  # 初始参数
                                   bias_attr=False)  # 输出中不包含偏置
        # Batch Noraml操作
        ...
        param_attr = ParamAttr(initializer=fluid.initializer.Normal(0.0, 0.02),
    ...
    def depthwise_conv_bn(self, input, filter_size=3, stride=1, padding=1):
        num_filters = input.shape[1]  # 取出通道数
        print("depthwise_conv_bn.num_filters:", num_filters)
    ...
    def down_sample(self, input, pool_size=2, pool_stride=2):
        ...
        return fluid.layers.pool2d(input=input,
                                   pool_type="max",
                                   pool_size=pool_size,
    ...
    def up_sample(self, input, scale=2):
        shape_nchw = fluid.layers.shape(input)  # 获取输入数据形状
        print(shape_nchw)
        shape_hw = fluid.layers.slice(shape_nchw, axes=[0],  # axes=0指定行的切片范围，1指定列的切片范围
                                      starts=[2], ends=[4])  # 取高度和宽度
        shape_hw.stop_gradient = True
        in_shape = fluid.layers.cast(shape_hw, dtype="int32")
        out_shape = in_shape * scale  # 计算输出形状
        out_shape.stop_gradient = True  # 梯度不进行回传
    ...
    def net(self, img):
        # darknet-tiny
        # yolo tiny没有使用res层(残差层)，只使用了两个不同尺度的yolo输出层
        ...
        # 循环创建basic_block组
        for i, stage_count in enumerate(stages):  # enumerate函数对可迭代对象自动编号
            if i == len(stages) - 1:  # 最后一组（后跟3组卷积）
                # 创建一个卷积层，添加到blocks中
...
def train():
    ...
    logger.info("Start train YOLOv3, train params: %s", str(train_params))  # 打印参数
    ...
    train_fetch_list = [loss.name]  # 获取的返回值
    ...
    if train_params['continue_train'] and os.path.exists(train_params['save_model_dir']):
        ...
        fluid.io.load_persistables(executor=exe,
                                   dirname=train_params['save_model_dir'],
                                   main_program=train_program)  # 加载增量模型
    ...
    # successive_count = 0
    # total_batch_count = 0
    valid_shresh = train_params["valid_thresh"]  # 有效阈值
    nms_thresh = train_params["nms_thresh"]  # 非极大值抑制阈值
    current_best_loss = 10000000000.0  # 控制训练参数
    ...
    for pass_id in range(train_params["num_epochs"]):
        ...
        for batch_id, data in enumerate(reader()):
            ...
            period = time.time() - t1  # 计算本次训练所花时间
            loss = np.mean(np.array(loss))  # 计算平均损失值
            total_loss += loss  # 计算总损失值
            # batch_id += 1
        ...
        # fluid.io.save_persistables(dirname=save_model_dir,
        #                            main_program=train_program,
        #                            executor=exe)

    # 训练完成后，保存增量模型
    logger.info("trainning till last epoch, end trainning.")
    ...
    fluid.io.save_persistables(dirname=save_model_dir,
                               main_program=train_program,
                               executor=exe)
    print("Save persistables ok:", save_model_dir)
